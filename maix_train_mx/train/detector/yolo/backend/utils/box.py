...
def draw_scaled_boxes(image, boxes, probs, labels, desired_size=400):
    ...
    img_cv2_PIL = Image.fromarray(cv2.cvtColor(img_scaled,cv2.COLOR_BGR2RGB))
    if boxes != []:
        boxes_scaled = boxes*scale_factor
        boxes_scaled = boxes_scaled.astype(np.int)
    else:
        boxes_scaled = boxes
    return draw_boxes(img_cv2_PIL, boxes_scaled, probs, labels)
...
def draw_boxes(image, boxes, probs, labels):
    ...
    for box, classes in zip(boxes, probs):
        x1, y1, x2, y2 = box
        draw.rectangle(xy=((x1,y1), (x2,y2)), fill=None, outline="#00d659", width=2)
        draw.text(xy=(x1, y1 - 13), text='{}:  {:.2f}'.format(labels[np.argmax(classes)], classes.max()), fill="#00d659")
    return image        
