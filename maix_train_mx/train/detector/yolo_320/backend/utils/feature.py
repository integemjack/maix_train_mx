...
def create_feature_extractor(architecture, alpha, input_size):
    """
    # Args
        architecture : str
        input_size : int
    ...
    if architecture == 'SqueezeNet':
        ...
        feature_extractor = MobileNetFeature(input_size, alpha, weights)
...
class MobileNetFeature(BaseFeatureExtractor):
    """docstring for ClassName"""
    def __init__(self, input_size,alpha,weights=False):
        input_image = Input(shape=(224, 320, 3))
        print(alpha)
        mobilenet = MobileNet(input_shape=(224,320,3),alpha = alpha,depth_multiplier = 1, dropout = 0.001, weights = 'imagenet', classes = 1000, include_top=False,backend=keras.backend, layers=keras.layers,models=keras.models,utils=keras.utils)
