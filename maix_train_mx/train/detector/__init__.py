...
class Detector(Train_Base):
    ...
    def _load_datasets_pascal_voc(self, datasets_img_dir ,datasets_xml_dir):
        ...
        for xml_path in xmls:
            ...
            img_file_name = xml_file_name.replace(".xml", ".jpg")
            result['filename'] = img_file_name
            # shape
            ...
            if not input_shape_checked:
                if not self._check_update_input_shape(img_shape) and not self.allow_reshape:
                    ...
                    self.log.i("not supported input size, supported: {}".format(self.support_shapes), [], None, None, None)
                    self.log.i("start resize voc.....")
                    if revoc.re(img_dir,ann_dir):
                        self.log.i("resize voc is ok.")
                    else:
                        self.log.e("ERROR : resize voc err")
                input_shape_checked = True
